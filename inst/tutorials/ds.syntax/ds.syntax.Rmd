...
Das Kapitel 3.3 im Buch beschreibt die Syntax von R.

## 1. Wertzuweisung

### 1.1 Demo

Wertzuweisungen erfolgen in R mit Hilfe des Zuweisungsoperators <-. Um einer Variablen x den Wert 3 zuzuweisen wäre daher die Anweisung a <- 24 korrekt. Der Wert einer Variablen kann angezeigt werden, indem eine Zeile mit dem Namen der Variablen vom R-Interpreter ausgeführt wird.
...
Lassen Sie sich anschließend die Variable "klausurnoten" anzeigen.
...
```{r 1_4, exercise=TRUE, dependson = c(-1, -2) }
klausurnoten <- ...
...
Geben Sie mean(klausurnoten) ein

```{r 2-setup, exercise=FALSE, exercise.setup="2", echo=FALSE, include=FALSE}
x <- 3
y <- 1 
klausurnoten <- c(x,y)
```
...
```{r 3_1-setup, exercise=FALSE, exercise.setup="3_1", echo=FALSE, include=FALSE}
x <- 3
y <- 1 
klausurnoten <- c(x,y)
```
...
Lassen Sie sich die Hilfe zu der Funktion mean anzeigen.
