...
Das Kapitel 4.1.3-4.1.4 behandelt Verteilungen ebenso wie Univariate, Bivariate und Multivariate Statistiken. Auch das Thema der Abhängigkeit von Variablen und deren Korrelation wird thematisiert. Nachfolgend wird vorgestellt, wie dies von R unterstützt wird.
...
```{r 1_0, exercise=FALSE, exercise.setup="1_0-setup"}
...
2. Wölbung (engl. Kurtosis) ist eine Maßzahl für die Steilheit bzw. „Spitzigkeit“ einer Verteilung, entweder (a) steilgipflig (supergaußförmig oder leptokurtisch) oder (b) flachgipflig (subgaußförmig oder platykurtisch). Die Funktion kurtosis() aus dem Paket e1071 berechnet die Exzess Kurtosis, also den Wert der Kurtosis -3.
...
Ob eine Verteilung schief ist, lässt sich auf Basis der Lagemaße Modus, Median und Mittelwert erkennen oder über die Funktion skewness() aus dem Paket e1071 berechnen.

Der Wert für die Wölbung beträgt im Falle der Normalverteilung 3. Liegt der Wert der Wölbung über 3, dann handelt es sich um eine schmalgipflige, spitze Verteilung und bei Werten kleiner als 3 um eine breitgipflige, flache Verteilung mit Beobachtungen nahe dem Mittelwert. Um das Ausmaß der Wölbung besser einschätzen zu können, wird die Wölbung einer Verteilung mit der Wölbung einer Normalverteilung verglichen. Für diesen Zweck berechnet man den Exzess oder die Exzess Kurtosis als Wölbung minus 3 der Wölbung einer Normalverteilung. Liegt der Wert der Exzess Kurtosis bei Null, so handelt es sich um eine näherungsweise Normalverteilung. Liegt der Wert über Null, dann handelt es sich um eine schmalgipflige, spitze Verteilung und bei Werten kleiner als Null um eine breitgipflige, flache Verteilung mit Beobachtungen nahe dem Mittelwert. Die Exzess Kurtosis wird von der Funktion kurtosis() aus dem Paket e1071 berechnet.
...
if (sp2.modus>sp2.median & sp2.median>sp2.mean) {
    paste("Schiefe",sp2.skewness,"< 0, Linke Schiefe: negative Schiefe, linksschief, rechtssteil")
} else if (sp2.modus<sp2.median & sp2.median<sp2.mean) {
    paste("Schiefe",sp2.skewness,"> 0, Rechte Schiefe: positive Schiefe, linkssteil, rechtsschief")
} else {
  paste("Schiefe",sp2.skewness,"Näherungsweise Normalverteilt")
}
#- Kurtosis-----------------------------------------------------------------------
...
```{r 5_3, exercise=FALSE, exercise.setup="5_3-setup"}
...
Auch bei der Überprüfung von Hypothesen können Fehler auftreten. Die Festlegung des Signifikanzniveaus hat darauf Einfluss (siehe Details dazu in Kapitel 4.3.3 im Buch):
![Hypothese](images/hypothese.jpg)
...
Es existiert eine signifikante (p-value < Signifikanzniveau von 0,05) starke Korrelation von 0.5769626 zwischen PS und Preis.
...
quiz(
  ...
  question("Welche der folgenden Aussagen sind korrekt?", allow_retry = TRUE, random_answer_order = TRUE,
    ...
    answer("Eine mit correlate() erzeugte Korrelationsmatrix zeigt alle Koeffizienten im Spiegelbild doppelt an.", message = "Die Anwendung der Funktion shave() und fashion() auf eine Korrelationsmatrix entfernt alle gespiegelten Werte.", correct = TRUE),
